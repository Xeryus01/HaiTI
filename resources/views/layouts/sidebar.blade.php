...
<aside class="fixed top-0 left-0 z-50 h-screen w-64 overflow-y-auto overflow-x-hidden bg-white dark:bg-dark-800 border-r border-gray-200 dark:border-gray-700 transition-all duration-300 ease-in-out" :style="{ transform: $store.sidebar.isOpen ? 'translateX(0)' : 'translateX(-100%)', width: '16rem' }">
    ...
    <div class="flex items-center justify-between px-4 py-4 sm:px-5.5 sm:py-6.5 lg:py-7.5">
        <a href="{{ route('dashboard') }}" class="block" @click="$store.sidebar.closeOnMobile()">
            <img src="{{ asset(file_exists(public_path('logo/full-logo.png')) ? 'logo/full-logo.png' : 'logo/logo.png') }}" alt="{{ config('app.name', 'TimCare') }}" class="h-12 w-auto object-contain sm:h-14" />
        </a>
    </div>
    ...
    <nav class="px-3 py-4 lg:px-5.5">
        <ul class="space-y-1.5">
            <!-- Dashboard -->
            <li>
                <a href="{{ route('dashboard') }}" class="group relative flex items-center gap-3 rounded-lg px-4 py-2.5 font-medium text-gray-500 dark:text-gray-400 {{ request()->routeIs('dashboard') ? 'bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400' : 'hover:bg-gray-100 dark:hover:bg-white/5' }}">
            ...
            <!-- Tickets -->
            <li>
                <a href="{{ route('tickets.index') }}" class="group relative flex items-center gap-3 rounded-lg px-4 py-2.5 font-medium text-gray-500 dark:text-gray-400 {{ request()->routeIs('tickets.*') ? 'bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400' : 'hover:bg-gray-100 dark:hover:bg-white/5' }}">
                    <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <li>
                <a href="{{ route('reservations.index') }}" class="group relative flex items-center gap-3 rounded-lg px-4 py-2.5 font-medium text-gray-500 dark:text-gray-400 {{ request()->routeIs('reservations.*') ? 'bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400' : 'hover:bg-gray-100 dark:hover:bg-white/5' }}">
                    ...
                    <span>Tiket ID Zoom Meeting</span>
                </a>
            </li>
            ...
            @if(auth()->user()->hasRole('Admin'))
            <li>
                <a href="{{ route('piket.index') }}" class="group relative flex items-center gap-3 rounded-lg px-4 py-2.5 font-medium text-gray-500 dark:text-gray-400 {{ request()->routeIs('piket.*') ? 'bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400' : 'hover:bg-gray-100 dark:hover:bg-white/5' }}">
                    <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <nav class="px-3 py-4 lg:px-5.5">
        <ul class="space-y-1.5">
            <!-- Profile -->
            <li>
                <a href="{{ route('profile.edit') }}" class="group relative flex items-center gap-3 rounded-lg px-4 py-2.5 font-medium text-gray-500 dark:text-gray-400 {{ request()->routeIs('profile.*') ? 'bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400' : 'hover:bg-gray-100 dark:hover:bg-white/5' }}">
                    <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M10 2C5.58172 2 2 5.58172 2 10C2 14.4183 5.58172 18 10 18C14.4183 18 18 14.4183 18 10C18 5.58172 14.4183 2 10 2ZM10 4C11.6569 4 13 5.34315 13 7C13 8.65685 11.6569 10 10 10C8.34315 10 7 8.65685 7 7C7 5.34315 8.34315 4 10 4ZM10 12C7.33579 12 4.25 13.2403 4.25 15.25C4.25 16.3856 7.21 17 10 17C12.79 17 15.75 16.3856 15.75 15.25C15.75 13.2403 12.6642 12 10 12Z"></path>
                    </svg>
                    <span>Profil</span>
