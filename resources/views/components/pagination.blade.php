    ...
    <div class="border-t border-gray-200 px-5 py-4 dark:border-gray-700 sm:px-6">
        <div class="flex flex-col gap-4 sm:gap-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="text-sm text-gray-600 dark:text-gray-400">
                Menampilkan <span class="font-medium">{{ $paginator->firstItem() ?? 0 }}</span> hingga 
                <span class="font-medium">{{ $paginator->lastItem() ?? 0 }}</span> dari 
        ...
        <div class="mt-4 flex items-center justify-center gap-1 flex-wrap">
            ...
            @else
                <a href="{{ $paginator->previousPageUrl() }}&per_page={{ request('per_page', 10) }}" 
            ...
            @if ($start > 1)
                <a href="{{ $paginator->url(1) }}&per_page={{ request('per_page', 10) }}" 
                    class="relative inline-flex items-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-dark-800 dark:text-gray-300 dark:hover:bg-white/10 transition-colors">
                    1
                </a>
                @if ($start > 2)
            ...
            @for ($i = $start; $i <= $end; $i++)
                ...
                @else
                    <a href="{{ $paginator->url($i) }}&per_page={{ request('per_page', 10) }}" 
                        class="relative inline-flex items-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-dark-800 dark:text-gray-300 dark:hover:bg-white/10 transition-colors">
                        {{ $i }}
                    </a>
                @endif
            ...
            @if ($end < $paginator->lastPage())
                @if ($end < $paginator->lastPage() - 1)
                    <span class="px-1 text-gray-500">...</span>
                @endif
                <a href="{{ $paginator->url($paginator->lastPage()) }}&per_page={{ request('per_page', 10) }}" 
                    class="relative inline-flex items-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-dark-800 dark:text-gray-300 dark:hover:bg-white/10 transition-colors">
                    {{ $paginator->lastPage() }}
                </a>
            @endif
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}&per_page={{ request('per_page', 10) }}" 
                    class="relative inline-flex items-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-dark-800 dark:text-gray-300 dark:hover:bg-white/10 transition-colors">
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
